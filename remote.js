    ...
    function generateCardNumber() {
      ...
      const parts = [ 4000 + rnd(500), 1000 + rnd(900), 1000 + rnd(900), 1000 + rnd(900) ];
      return parts.join(' ');
    }
    ...
    function addBalance(v) { const add = Math.max(0, v); state.balance += add; state.totalEarned += add; }
    function spend(cost) { if (state.balance >= cost) { state.balance -= cost; return true; } return false; }
    ...
    function watchAd() {
      const dk = todayKey();
      if (!state.ad.levelResetAt || now() > state.ad.levelResetAt) { state.ad.level = 1; }
      state.ad.level += 1;
      ...
      return new Promise((resolve)=>{
        try {
          if (window.admob && window.admob.rewarded) {
            window.admob.rewarded.load({ id: (state.ads && state.ads.admobRewardUnitId) || undefined })
              .then(()=> window.admob.rewarded.show())
              .then(()=> resolve())
              .catch(()=>{ toast('Reklam hatası'); resolve(); });
          } else { toast('Reklam servisi kullanılamıyor'); resolve(); }
    ...
    function mmss(s) { s = Math.max(0, s|0); const m = (s/60)|0; const r = s%60; return String(m).padStart(2,'0')+':'+String(r).padStart(2,'0'); }
    ...
    function renderFootball() {
      ...
      qsa('[data-team]').forEach(btn=> btn.onclick = ()=>{
        ...
        if (!tm) return;
        if (act==='buy') { const cost = tm.owned ? (200 + tm.level * 220) : (500 + tm.baseMatch*10); if (spend(cost)) { tm.owned = true; tm.level = (tm.level||0)+1; } }
        else if (act==='transfer') { const c = 100 + (tm.quality||1) * 120; if (spend(c)) tm.quality = (tm.quality||1)+1; }
        else if (act==='stadium') { const c = 300 + (tm.stadium||0) * 300; if (spend(c)) tm.stadium = (tm.stadium||0)+1; }
        else if (act==='training') { const c = 240 + (tm.training||0) * 240; if (spend(c)) tm.training = (tm.training||0)+1; }
        else if (act==='sponsor') { const c = 200 + (tm.sponsor||0) * 260; if (spend(c)) tm.sponsor = (tm.sponsor||0)+1; }
        const d = state.tasks.daily.find(x=>x.id==='d_invest'); if (d) { ensureDaily(d); d.progress = clamp(d.progress+1, 0, d.goal); }
    ...
    function renderBankPanels(panels) {
      ...
      qs('#btnDep').onclick = ()=> { const v = parseFloat(qs('#amtChk').value||'0'); if (Number.isFinite(v) && v>0 && spend(v)) { state.bank.checking += v; renderInvestments(); saveState(false); } };
      qs('#btnWit').onclick = ()=> { const v = parseFloat(qs('#amtChk').value||'0'); if (Number.isFinite(v) && v>0 && state.bank.checking>=v) { state.bank.checking -= v; state.balance += v; renderInvestments(); saveState(false); } };
      ...
      qs('#btnTDEarly').onclick = ()=>{
        ...
        let payout;
        if (now() >= td.start + (td.durationMs||0)) { payout = td.amount + td.amount * td.rate; }
        else { payout = td.amount * (1 - td.penalty); }
    ...
    // WebAudio click/nav sounds (cross-browser safe)
    ...
    function playClick(){ playTone(660, 0.06); }
    function playNav(){ playTone(440, 0.05); }
    ...
    function openSettings(show) { qs('#settingsModal').classList.toggle('show', !!show); }
    ...
    function applyOfflineGains() {
      ...
      const seconds = clamp(((now()-last)/1000)|0, 0, 8*60*60);
    ...
    function setupStartOverlay() {
      ...
      qs('#btnStartNew').onclick = ()=> {
        if (hasSave) { if (!confirm('Kayıtlı oyun var, sıfırdan başlamak ister misiniz?')) return; }
